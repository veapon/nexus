...
if ($type == 'invite')
{
	...
	$invitee = htmlspecialchars($inv["invitee"]);
	if (!$inv)
...
show_image_code ();
?>
<tr><td class=rowhead><?php echo $lang_signup['row_email_address'] ?></td>
<?php if($type=='invite')
		{
			print("<td class=rowfollow align=left>".$invitee);
			print("<input type=\"hidden\" name=\"email\" value=\"".$invitee." \" />");
		}
else
	{
		print("<td class=rowfollow align=left><input type=text style=width: 200px name=email />");
	}
?>
